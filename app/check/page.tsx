...
export default function DailyCheckPage() {
  ...
  const handleSaveIncoming = async (productId: number, currentYestBal: number, currentEvening: number | null) => {
    if (!currentBranch) return;
    try { const { error } = await supabase.from('daily_stock_checks').upsert({ product_id: productId, check_date: todayForDB, yesterday_balance: currentYestBal, incoming: editIncomingCount === '' ? 0 : parseFloat(editIncomingCount), evening_counted: currentEvening, branch_id: currentBranch.id }, { onConflict: 'check_date, product_id' })
  ...
  const handleSaveEvening = async (productId: number, currentYestBal: number, currentInc: number) => {
    ...
    try {
      ...
      if (newEve !== null) {
        const currentDate = new Date(); currentDate.setDate(currentDate.getDate() + 1); const nextDateStr = `${currentDate.getFullYear()}-${String(currentDate.getMonth() + 1).padStart(2, '0')}-${String(currentDate.getDate()).padStart(2, '0')}`;
        await supabase.from('daily_stock_checks').upsert({ product_id: productId, check_date: nextDateStr, yesterday_balance: newEve, branch_id: currentBranch.id }, { onConflict: 'check_date, product_id' });
      }
      setEditingEveningId(null); fetchData()
    } catch (error: any) { alert('❌ อัปเดตไม่สำเร็จ: ' + error.message) }
  }

  const handleSaveOrderQty = async (checkId: number | null, productId: number) => {
    if (!currentBranch) return;
    const qty = editOrderQty === '' ? null : parseFloat(editOrderQty);
    try {
      if (checkId) { await supabase.from('daily_stock_checks').update({ actual_order_qty: qty }).eq('id', checkId); } 
      else { await supabase.from('daily_stock_checks').insert({ check_date: todayForDB, branch_id: currentBranch.id, product_id: productId, actual_order_qty: qty, yesterday_balance: 0, incoming: 0 }); }
      setEditingOrderId(null); fetchData();
    } catch (e) { alert('❌ อัปเดตยอดสั่งไม่สำเร็จ'); }
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-6 sm:mb-8"><h1 className="text-2xl sm:text-3xl font-bold text-[#df2323]">เช็คสต๊อกรายวัน <span className="text-gray-500 text-lg ml-2">({currentBranch.name})</span></h1><div className="bg-white px-4 sm:px-5 py-2 rounded-full border border-gray-200 text-sm font-semibold text-gray-700 shadow-sm w-fit">{today.toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: 'numeric' })}</div></div>
                ...
                {tableRows.length === 0 ? (<tr><td colSpan={7} className="p-16 text-gray-400">ยังไม่มีรายการสินค้าในระบบ</td></tr>) : (
                  Object.entries(groupedRows).map(([category, items]) => (
                    <React.Fragment key={category}>
                      <tr ref={(el) => { categoryRefs.current[category] = el; }} className="bg-gray-100 border-y border-gray-200"><td className="p-2 sm:p-3 pl-4 sm:pl-6 text-left font-bold text-gray-800 text-xs sm:text-sm sticky left-0 z-20 bg-gray-100 border-r border-gray-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] whitespace-nowrap">📂 {category}</td><td colSpan={6} className="bg-gray-100"></td></tr>
                      {items.map((item) => {
                        ...
                        return (
                          <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/80 transition-colors group">
                            <td className="p-2 sm:p-4 text-left font-semibold text-gray-800 bg-white group-hover:bg-gray-50/80 sticky left-0 z-10 border-r border-gray-100 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] transition-colors w-[110px] min-w-[110px] sm:w-auto sm:min-w-[200px] whitespace-normal break-words sm:whitespace-nowrap">
                              <div className="text-[13px] sm:text-[15px] leading-tight">{item.name}</div>
                              <div className="text-[9px] sm:text-[11px] text-gray-500 font-normal mt-1 flex flex-col sm:flex-row sm:gap-2 leading-none sm:leading-normal">
                                <span>Max: {item.max_limit !== null ? Number(Number(item.max_limit).toFixed(1)) : '-'}</span>
                                <span>Min: {item.min_limit !== null ? Number(Number(item.min_limit).toFixed(1)) : '-'}</span>
                              </div>
                            </td>
                            <td className="p-2 sm:p-4 text-yellow-900 font-semibold bg-yellow-50/30 text-sm sm:text-base">{Number(item.yesterday_balance.toFixed(1))}</td>
                            <td className="p-2 sm:p-4 text-yellow-900 font-semibold bg-yellow-50/30">
                              {editingIncomingId === item.id ? (
                                <div className="flex flex-col items-center gap-1.5"><input type="number" step="any" autoFocus className="w-14 sm:w-20 border-2 border-[#facc15] rounded-xl p-1.5 sm:p-2 text-center font-bold text-gray-900 focus:outline-none shadow-sm text-sm" value={editIncomingCount} onChange={(e) => setEditIncomingCount(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSaveIncoming(item.id, item.yesterday_balance, item.evening_counted)} /><div className="flex gap-1"><button onClick={() => handleSaveIncoming(item.id, item.yesterday_balance, item.evening_counted)} className="bg-yellow-600 text-white text-[9px] sm:text-[11px] px-2 py-1 rounded-md font-bold">บันทึก</button><button onClick={() => setEditingIncomingId(null)} className="bg-yellow-100 text-yellow-800 text-[9px] sm:text-[11px] px-2 py-1 rounded-md font-bold">ยกเลิก</button></div></div>
                              ) : (
                                <div className="flex flex-col items-center justify-center"><span className="text-base sm:text-lg">+{item.incoming}</span><button onClick={() => { setEditingIncomingId(item.id); setEditIncomingCount(String(item.incoming)); }} className="mt-1 bg-white border border-yellow-300 text-yellow-700 hover:bg-yellow-100 text-[9px] sm:text-[10px] px-2.5 sm:px-3 py-1 rounded-full transition-colors flex items-center gap-1 font-semibold shadow-sm">✏️ แก้ไข</button></div>
                              )}
                            </td>
                            <td className="p-2 sm:p-4 text-yellow-950 font-bold text-lg sm:text-xl bg-yellow-100/40">{totalAvailable}</td>
                            <td className="p-2 sm:p-4 bg-blue-50/20 text-blue-700">
                              {editingEveningId === item.id ? (
                                <div className="flex flex-col items-center gap-1.5"><input type="number" step="any" autoFocus className="w-16 sm:w-24 border-2 border-blue-400 rounded-xl p-1.5 sm:p-2.5 text-center font-bold text-gray-900 focus:outline-none shadow-md text-sm" value={editEveningCount} onChange={(e) => setEditEveningCount(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSaveEvening(item.id, item.yesterday_balance, item.incoming)} /><div className="flex gap-1 sm:gap-2 mt-1"><button onClick={() => handleSaveEvening(item.id, item.yesterday_balance, item.incoming)} className="bg-blue-600 text-white text-[10px] sm:text-xs px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg font-bold">บันทึก</button><button onClick={() => setEditingEveningId(null)} className="bg-gray-200 text-gray-600 text-[10px] sm:text-xs px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg font-bold">ยกเลิก</button></div></div>
                              ) : (
                                <div className="flex flex-col items-center justify-center">
                                  {item.evening_counted === null ? (
                                    <button onClick={() => { setEditingEveningId(item.id); setEditEveningCount(''); }} className="bg-blue-500 hover:bg-blue-600 text-white text-xs sm:text-sm px-3 sm:px-5 py-1.5 sm:py-2 rounded-full transition-colors flex items-center gap-1 sm:gap-2 font-bold shadow-sm">✍️ ลงยอดนับ</button>
                                  ) : (
                                    <><span className="text-2xl sm:text-3xl font-bold text-blue-700 mb-1">{eveningCounted}</span><button onClick={() => { setEditingEveningId(item.id); setEditEveningCount(String(item.evening_counted)); }} className="bg-white border border-blue-200 text-blue-600 hover:bg-blue-50 text-[9px] sm:text-[11px] px-2.5 sm:px-3 py-1 rounded-full transition-colors flex items-center gap-1.5 font-semibold shadow-sm">✏️ แก้ไขยอด</button></>
                                  )}
                                </div>
                              )}
                            </td>
                            <td className="p-2 sm:p-4 text-gray-700 font-semibold text-lg sm:text-xl">{usedAmount}</td>
                            ...
                            <td className={`p-2 sm:p-4 font-bold text-lg bg-red-50/20 border-l border-[#fecaca] align-middle ${needsOrder ? 'text-[#df2323]' : 'text-gray-400'}`}>
                              {editingOrderId === item.id ? (
                                <div className="flex flex-col items-center gap-1.5">
                                  <input type="number" step="any" autoFocus className="w-14 sm:w-16 border-2 border-[#df2323] rounded-lg p-1 sm:p-1.5 text-center font-bold text-[#df2323] focus:outline-none shadow-sm text-sm sm:text-base" value={editOrderQty} onChange={(e) => setEditOrderQty(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSaveOrderQty(item.check_id, item.id)} />
                                  <div className="flex gap-1">
                                    <button onClick={() => handleSaveOrderQty(item.check_id, item.id)} className="bg-[#df2323] text-white text-[10px] px-2 py-1 rounded font-bold shadow-sm">บันทึก</button>
                                ...
                                <div className="flex flex-col items-center justify-center">
                                  ...
                                  {hasLinkedItems && item.evening_counted !== null && (<div className="text-[9px] sm:text-[10px] text-gray-500 font-medium mt-1 leading-tight">รวมยอดเตรียม<br/>({Number(totalCombinedStock.toFixed(1))} {item.unit})</div>)}
                                </div>
                              )}
                            </td>
                          </tr>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 sm:p-8 animate-in fade-in duration-200">
            <div className="bg-[#f8f9fa] rounded-3xl shadow-2xl w-[calc(100%-2rem)] sm:w-[calc(100%-4rem)] max-w-4xl max-h-[90vh] flex flex-col overflow-hidden mx-auto">
              <div className="bg-[#4f46e5] p-4 sm:p-5 px-5 sm:px-6 flex justify-between items-center text-white flex-shrink-0">
                <h2 className="text-lg sm:text-xl font-bold flex items-center gap-2">📦 เลือกรายการสินค้า</h2>
              ...
              <div className="p-3 sm:p-4 bg-white border-b border-gray-200 flex-shrink-0">
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 pl-3 sm:pl-4 flex items-center text-gray-400">🔍</span>
                  <input type="text" placeholder="ค้นหาสินค้า..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 sm:pl-11 pr-4 py-2.5 sm:py-3 border-2 border-gray-200 rounded-xl sm:rounded-2xl focus:outline-none focus:border-[#4f46e5] text-gray-700 font-medium text-sm sm:text-base" autoFocus />
                </div>
              </div>
                  ...
                  {Object.keys(groupedModalProducts).length === 0 ? (
                    ...
                    Object.entries(groupedModalProducts).map(([category, items]) => (
                      <div key={category} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden h-fit">
                        <div className="bg-gray-50 border-b border-gray-200 px-3 sm:px-4 py-2.5 sm:py-3 font-bold text-gray-700 text-xs sm:text-sm flex items-center gap-2">
                          <span className="text-[#4f46e5]">📚</span> หมวดหมู่: {category}
                        </div>
                        <div className="p-2 sm:p-3 flex flex-col gap-2 sm:gap-2.5 bg-gray-50/30">
                          {items.map(item => (
                            <button key={item.id} onClick={() => handleSelectProductFromModal(item.id, item.name)} className="flex justify-between items-center w-full px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl bg-white border border-gray-100 hover:border-indigo-200 hover:bg-indigo-50 transition-all group text-left shadow-[0_2px_4px_rgba(0,0,0,0.02)] hover:shadow-md">
                              <span className="font-bold text-gray-700 text-sm sm:text-base group-hover:text-indigo-700">{item.name}</span>
                              <span className="bg-[#4f46e5] text-white text-[10px] sm:text-xs px-3 sm:px-4 py-1 sm:py-1.5 rounded-full font-semibold shadow-sm group-hover:bg-indigo-600 transition-colors">เลือก</span>
                            </button>
                          ))}
                        </div>
                      </div>
