...
export default function DailyCheckPage() {
  ...
  const tableRows = products.map(product => {
    ...
    if (latestCheck) {
      if (latestCheck.evening_counted !== null) defaultLatestBalance = latestCheck.evening_counted;
      else defaultLatestBalance = latestCheck.yesterday_balance + latestCheck.incoming;
    }
  ...
  const groupedRows = tableRows.reduce((acc, row) => {
    ...
    if (!acc[cat]) acc[cat] = []; acc[cat].push(row); return acc;
  ...
  const handleSelectProductFromModal = (id: number, name: string) => {
  ...
  const handleMorningSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('daily_stock_checks').upsert({ product_id: parseInt(selectedProductId), check_date: todayForDB, yesterday_balance: yesterdayBalance ? parseFloat(yesterdayBalance) : 0, incoming: incoming ? parseFloat(incoming) : 0 }, { onConflict: 'check_date, product_id' })
  ...
  const handleSaveIncoming = async (productId: number, currentYestBal: number, currentEvening: number | null) => {
    try {
      const { error } = await supabase.from('daily_stock_checks').upsert({ product_id: productId, check_date: todayForDB, yesterday_balance: currentYestBal, incoming: editIncomingCount === '' ? 0 : parseFloat(editIncomingCount), evening_counted: currentEvening }, { onConflict: 'check_date, product_id' })
  ...
  const handleSaveEvening = async (productId: number, currentYestBal: number, currentInc: number) => {
    try {
      const { error } = await supabase.from('daily_stock_checks').upsert({ product_id: productId, check_date: todayForDB, yesterday_balance: currentYestBal, incoming: currentInc, evening_counted: editEveningCount === '' ? null : parseFloat(editEveningCount) }, { onConflict: 'check_date, product_id' })
      if (error) throw error; setEditingEveningId(null); fetchData()
    } catch (error: any) { alert('❌ อัปเดตไม่สำเร็จ: ' + error.message) }
  }

  const filteredModalProducts = tableRows.filter(p => 
    p.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    p.categoryName.toLowerCase().includes(searchQuery.toLowerCase())
  );
  
  const groupedModalProducts = filteredModalProducts.reduce((acc, row) => {
    const cat = row.categoryName;
    if (!acc[cat]) acc[cat] = []; acc[cat].push(row); return acc;
  }, {} as Record<string, typeof tableRows>);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
          ...
          <form onSubmit={handleMorningSubmit} className="p-4 sm:p-8 flex flex-col sm:flex-row flex-wrap items-stretch sm:items-end gap-4 sm:gap-6">
            <div className="flex-1 min-w-full sm:min-w-[250px]">
              ...
              <div 
                ...
                className="w-full border border-gray-200 rounded-xl p-3 sm:p-3.5 focus:outline-none focus:border-[#facc15] bg-white shadow-inner transition-colors text-sm sm:text-base cursor-pointer flex justify-between items-center"
              >
                <span className={selectedProductName ? 'text-gray-900 font-bold' : 'text-gray-400'}>
                  {selectedProductName ? `🛒 ${selectedProductName}` : '-- คลิกเพื่อเลือกสินค้า --'}
                </span>
            ...
            <div className="w-full sm:w-40">
              ...
              <input type="number" step="any" value={yesterdayBalance} onChange={(e) => setYesterdayBalance(e.target.value)} className="w-full border border-gray-200 rounded-xl p-3 sm:p-3.5 focus:outline-none focus:border-[#facc15] shadow-inner transition-colors bg-gray-50 text-sm sm:text-base" placeholder="0" />
            </div>
            <div className="w-full sm:w-40">
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">ของเข้าวันนี้</label>
              <input type="number" step="any" required value={incoming} onChange={(e) => setIncoming(e.target.value)} className="w-full border border-gray-200 rounded-xl p-3 sm:p-3.5 focus:outline-none focus:border-[#facc15] shadow-inner transition-colors text-sm sm:text-base" placeholder="รับมา" />
            </div>
            <button type="submit" disabled={isSubmitting} className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 sm:px-8 py-3 sm:py-3.5 rounded-xl font-bold shadow-md transition-colors disabled:opacity-50 h-[50px] sm:h-[58px] w-full sm:w-auto flex-shrink-0">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[75vh] min-h-[500px]">
          ...
          <div className="bg-[#2563eb] p-4 px-8 text-white font-bold flex items-center justify-between gap-3 flex-shrink-0 relative z-20">
                    ...
                    <React.Fragment key={category}>
                      ...
                      {items.map((item) => {
                        ...
                        if (item.evening_counted !== null && item.min_limit !== null && item.max_limit !== null) {
                          if (item.evening_counted <= item.min_limit) { orderAmount = Number((item.max_limit - item.evening_counted).toFixed(1)); needsOrder = orderAmount > 0; }
                          else { orderAmount = 0; }
                        }
                          ...
                          <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/80 transition-colors group">
                            ...
                            <td className="p-4 text-yellow-900 font-semibold bg-yellow-50/30">
                              {editingIncomingId === item.id ? (
                                <div className="flex flex-col items-center gap-2">
                                  <input type="number" step="any" autoFocus className="w-20 border-2 border-[#facc15] rounded-xl p-2 text-center font-bold text-gray-900 focus:outline-none shadow-sm" value={editIncomingCount} onChange={(e) => setEditIncomingCount(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSaveIncoming(item.id, item.yesterday_balance, item.evening_counted)} />
                                  <div className="flex gap-1"><button onClick={() => handleSaveIncoming(item.id, item.yesterday_balance, item.evening_counted)} className="bg-yellow-600 text-white text-[11px] px-2 py-1 rounded-md font-bold">บันทึก</button><button onClick={() => setEditingIncomingId(null)} className="bg-yellow-100 text-yellow-800 text-[11px] px-2 py-1 rounded-md font-bold">ยกเลิก</button></div>
                                </div>
                              ) : (
                                <div className="flex flex-col items-center justify-center"><span className="text-lg">+{item.incoming}</span><button onClick={() => { setEditingIncomingId(item.id); setEditIncomingCount(String(item.incoming)); }} className="mt-1 bg-white border border-yellow-300 text-yellow-700 hover:bg-yellow-100 text-[10px] px-3 py-1 rounded-full transition-colors flex items-center gap-1 font-semibold shadow-sm">✏️ แก้ไข</button></div>
                              )}
                            </td>
                            ...
                            <td className="p-4 bg-blue-50/20 text-blue-700">
                              {editingEveningId === item.id ? (
                                <div className="flex flex-col items-center gap-2">
                                  <input type="number" step="any" autoFocus className="w-24 border-2 border-blue-400 rounded-xl p-2.5 text-center font-bold text-gray-900 focus:outline-none shadow-md" value={editEveningCount} onChange={(e) => setEditEveningCount(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSaveEvening(item.id, item.yesterday_balance, item.incoming)} />
                                  <div className="flex gap-2 mt-1"><button onClick={() => handleSaveEvening(item.id, item.yesterday_balance, item.incoming)} className="bg-blue-600 text-white text-xs px-4 py-2 rounded-lg font-bold">บันทึก</button><button onClick={() => setEditingEveningId(null)} className="bg-gray-200 text-gray-600 text-xs px-4 py-2 rounded-lg font-bold">ยกเลิก</button></div>
                                </div>
                              ) : (
                                <div className="flex flex-col items-center justify-center">
                                  {item.evening_counted === null ? (
                                    <button onClick={() => { setEditingEveningId(item.id); setEditEveningCount(''); }} className="bg-blue-500 hover:bg-blue-600 text-white text-sm px-5 py-2 rounded-full transition-colors flex items-center gap-2 font-bold shadow-sm">✍️ ลงยอดนับ</button>
                                  ) : (
                                    <><span className="text-3xl font-bold text-blue-700 mb-1">{eveningCounted}</span><button onClick={() => { setEditingEveningId(item.id); setEditEveningCount(String(item.evening_counted)); }} className="bg-white border border-blue-200 text-blue-600 hover:bg-blue-50 text-[11px] px-3 py-1 rounded-full transition-colors flex items-center gap-1.5 font-semibold shadow-sm">✏️ แก้ไขยอด</button></>
                                  )}
                                </div>
                              )}
                            </td>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-[#f8f9fa] rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
              <div className="bg-[#4f46e5] p-5 px-6 flex justify-between items-center text-white flex-shrink-0">
                ...
                <button onClick={() => setIsModalOpen(false)} className="bg-white/20 hover:bg-white/30 rounded-full w-8 h-8 flex items-center justify-center transition-colors">
                  ...
                  {Object.keys(groupedModalProducts).length === 0 ? (
                    ...
                    Object.entries(groupedModalProducts).map(([category, items]) => (
                      <div key={category} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden h-fit">
                        <div className="bg-gray-50 border-b border-gray-200 px-4 py-3 font-bold text-gray-700 text-sm flex items-center gap-2"><span className="text-[#4f46e5]">📚</span> หมวดหมู่: {category}</div>
                        <div className="p-2 flex flex-col gap-1">
                          {items.map(item => (
                            <button key={item.id} onClick={() => handleSelectProductFromModal(item.id, item.name)} className="flex justify-between items-center w-full px-4 py-3 rounded-xl hover:bg-indigo-50 transition-colors group text-left">
                              <span className="font-bold text-gray-800 group-hover:text-indigo-700">{item.name}</span>
                              <span className="bg-[#4f46e5] text-white text-xs px-3 py-1.5 rounded-full font-semibold shadow-sm group-hover:bg-indigo-600 transition-colors">เลือก</span>
                            </button>
                          ))}
                        </div>
                      </div>
