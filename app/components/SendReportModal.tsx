...
export default function SendReportModal({ isOpen, onClose, selectedDate, onDateChange, fullReportData }: SendReportModalProps) {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (result.success) {
        alert('ส่งรายงาน PDF ผ่านอีเมลสำเร็จเรียบร้อยครับ!')
        onClose()
      } else {
        alert('ส่งอีเมลไม่สำเร็จ: ' + result.error)
      }
    } catch (error) {
      alert('เกิดข้อผิดพลาดในการเชื่อมต่อระบบส่งอีเมล')
    } finally {
          ...
          <div className="mb-4 relative">
            ...
            <div className="relative">
              ...
              <button 
                type="button"
                onMouseDown={(e) => {
                  e.preventDefault(); // ป้องกันไม่ให้ input หลุดโฟกัส
              ...
              {showEmailList && recentEmails.length > 0 && (
                <div 
                  ...
                  onMouseDown={(e) => e.preventDefault()} // ป้องกันไม่ให้ input หลุดโฟกัสตอนคลิกกล่อง
                >
