...
export default function SendReportModal({ isOpen, onClose, selectedDate, onDateChange, fullReportData }: SendReportModalProps) {
  ...
  // 🔴 ฟังก์ชันเช็คความพร้อม (เพิ่มการตรวจสอบรูปแบบอีเมล)
  const handlePreSend = () => {
    const trimmedEmail = email.trim();
    
    // 1. เช็คว่าปล่อยว่างไหม
    if (!trimmedEmail) {
      setWarningMsg('กรุณากรอก "อีเมลผู้รับ" ก่อนทำการส่งรายงานครับ')
      return
    }

    // 2. เช็ครูปแบบอีเมลว่าถูกต้องไหม (ต้องมี @ และ .)
    ...
    // ผ่านฉลุย ให้เปิดหน้าต่าง Confirm
    setShowConfirm(true)
  }
    ...
    try {
      ...
      const res = await fetch('/api/send-report', {
        ...
        body: JSON.stringify({
          ...
          recipientEmail: email.trim(), // ส่งแบบตัดช่องว่างซ้ายขวาออก
          message: message,
  ...
  return (
    <>
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
        {/* 🔴 ลดขนาดความกว้างลงมาเป็น max-w-3xl และปรับ max-h ให้กระชับขึ้น */}
          ...
          <div className="w-full md:w-2/3 p-6 md:p-8 overflow-y-auto custom-scrollbar flex flex-col">
            ...
            <div className="flex flex-col gap-5 flex-1">
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="ใส่อีเมลผู้รับ เช่น manager@nuansuki.com"
                  className="w-full border-2 border-gray-200 rounded-xl p-3 focus:outline-none focus:border-[#df2323] transition-colors" 
                />
              </div>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <textarea 
                  ...
                  className="w-full border border-gray-200 rounded-xl p-3 focus:outline-none focus:border-[#df2323] transition-colors resize-none flex-1 min-h-[120px] text-sm text-gray-600 leading-relaxed" 
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-6 pt-5 border-t border-gray-100">
              ...
              <button 
                ...
                className="flex-1 bg-[#df2323] hover:bg-[#be123c] text-white font-bold py-3 rounded-xl shadow-md transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {isSubmitting ? (
                  <>กำลังเตรียมข้อมูล...</>
                ) : (
                  <><span>🚀</span> ยืนยันการส่ง</>
                )}
              </button>
            </div>
          </div>
      ...
      {warningMsg && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 transform transition-all text-center border-t-8 border-orange-400">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">แจ้งเตือน</h3>
            <p className="text-gray-600 mb-8 text-sm">{warningMsg}</p>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 transform transition-all text-center">
            ...
            <p className="text-gray-600 mb-6 text-sm">
              คุณต้องการส่งรายงานของวันที่<br/>
              <span className="font-bold text-[#df2323]">{fullThDate}</span><br/>
              ไปที่อีเมล <span className="font-bold text-blue-600">{email}</span><br/>
              ใช่หรือไม่?
            </p>
            <div className="flex gap-3">
              <button onClick={() => setShowConfirm(false)} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3.5 rounded-xl transition-colors">ตรวจสอบอีกครั้ง</button>
              <button onClick={executeSend} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl shadow-md transition-colors">ส่งเลย!</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 transform transition-all text-center">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-2">ส่งรายงานสำเร็จ!</h3>
            <p className="text-gray-500 mb-8 text-sm">อีเมลถูกส่งไปยัง <span className="font-semibold text-gray-700">{email}</span> เรียบร้อยแล้ว</p>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 transform transition-all text-center border-t-8 border-red-500">
            <div className="w-20 h-20 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-5 shadow-inner">
              <svg className="w-10 h-10 text-[#df2323]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M6 18L18 6M6 6l12 12" /></svg>
            </div>
            <h3 className="text-2xl font-bold text-gray-900 mb-2">ส่งไม่สำเร็จ</h3>
