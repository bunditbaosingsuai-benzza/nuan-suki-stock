...
export default function ProductsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: prodError } = await supabase.from('products').insert([{ name, category_id: categoryId, unit, max_limit: maxLimit ? parseFloat(maxLimit) : null, min_limit: minLimit ? parseFloat(minLimit) : null, raw_material_id: rawMaterialId ? parseInt(rawMaterialId) : null }])
      if (prodError) throw prodError
      setName(''); setCategoryName(''); setUnit(''); setMaxLimit(''); setMinLimit(''); setRawMaterialId('')
      fetchProducts()
  ...
  const handleSaveProduct = async (id: number) => {
    try {
      ...
      const { error } = await supabase.from('products').update({ name: editName, category_id: categoryId, unit: editUnit, max_limit: editMaxLimit === '' ? null : parseFloat(editMaxLimit), min_limit: editMinLimit === '' ? null : parseFloat(editMinLimit), raw_material_id: editRawMaterialId === '' ? null : parseInt(editRawMaterialId) }).eq('id', id)
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto relative">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-8 mb-8">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row flex-wrap items-stretch lg:items-end gap-4 lg:gap-6">
          ...
          <div className="flex-1 min-w-[150px]">
            ...
            <input type="text" required value={categoryName} onChange={(e) => setCategoryName(e.target.value)} className="w-full border border-[#df2323] rounded-xl p-3 focus:outline-none focus:ring-1 focus:ring-[#df2323] transition-colors text-sm sm:text-base" placeholder="เช่น เครื่องปรุง" />
          </div>
          <div className="flex-1 min-w-[150px]">
            <label className="block text-sm font-bold text-gray-700 mb-2">หน่วยนับ</label>
            <input type="text" required value={unit} onChange={(e) => setUnit(e.target.value)} className="w-full border border-[#df2323] rounded-xl p-3 focus:outline-none focus:ring-1 focus:ring-[#df2323] transition-colors text-sm sm:text-base" placeholder="เช่น แกลลอน" />
          </div>
          <div className="w-full sm:w-32">
            <label className="block text-sm font-bold text-gray-700 mb-2">ห้ามเกิน</label>
            <input type="number" step="any" value={maxLimit} onChange={(e) => setMaxLimit(e.target.value)} className="w-full border border-gray-200 rounded-xl p-3 focus:outline-none focus:border-[#df2323] transition-colors text-sm sm:text-base" placeholder="15" />
          </div>
          <div className="w-full sm:w-32">
            <label className="block text-sm font-bold text-gray-700 mb-2">ขั้นต่ำ</label>
            <input type="number" step="any" value={minLimit} onChange={(e) => setMinLimit(e.target.value)} className="w-full border border-gray-200 rounded-xl p-3 focus:outline-none focus:border-[#df2323] transition-colors text-sm sm:text-base" placeholder="5" />
          </div>
          <div className="flex-1 min-w-full sm:min-w-[200px]">
          ...
          <button type="submit" disabled={isSubmitting} className="bg-[#059669] hover:bg-[#047857] text-white px-6 sm:px-8 py-3 sm:py-3.5 rounded-xl font-bold shadow-md transition-colors disabled:opacity-50 h-[52px] w-full lg:w-full">
      ...
      {/* ========================================================= */}
      {/* 🔴 ตารางสินค้า (เพิ่มล็อกหัวตาราง & ล็อกคอลัมน์ชื่อ) */}
      {/* ========================================================= */}
          ...
          <table className="w-full text-center border-collapse">
            <thead className="sticky top-0 z-30 shadow-sm">
              <tr className="text-sm border-b border-gray-200 bg-[#f8f9fa]">
                <th className="p-5 font-bold text-gray-700 text-left sticky left-0 z-40 bg-[#f8f9fa] border-r border-gray-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">ชื่อสินค้า / หมวดหมู่</th>
                    ...
                    {items.map((product) => (
                      <tr key={product.id} className="border-b border-gray-100 hover:bg-gray-50/80 transition-colors group">
                        {editingProductId === product.id ? (
                          <>
                            <td className="p-3 text-left sticky left-0 z-20 bg-white group-hover:bg-gray-50/80 border-r border-gray-100 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] transition-colors">
                          ...
                          <>
                            <td className="p-5 text-left sticky left-0 z-20 bg-white group-hover:bg-gray-50/80 border-r border-gray-100 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] transition-colors">
                            ...
                            <td className="p-5 font-semibold text-blue-600">{product.raw_material_id ? products.find(p => p.id === product.raw_material_id)?.name || '-' : '-'}</td>
