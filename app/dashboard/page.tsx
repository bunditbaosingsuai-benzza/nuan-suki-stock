  ...
  const fullReportData = dashboardRows.map(row => {
    ...
    if (row.evening_counted !== null && row.min_limit !== null && row.max_limit !== null) {
      ...
      if (totalCombinedStock <= row.min_limit) { 
        calcOrder = Math.ceil(row.max_limit - totalCombinedStock); 
      }
    }
  ...
  const handleSaveOrderQty = async (checkId: number | null, productId: number) => {
    ...
    try {
      if (checkId) {
        ...
        await supabase.from('daily_stock_checks').insert({ check_date: selectedDate, branch_id: currentBranch?.id, product_id: productId, actual_order_qty: qty, yesterday_balance: 0, incoming: 0 });
      }
      setEditingOrderId(null); fetchData();
    } catch (e) { setTelegramModal({ isOpen: true, type: 'error', message: 'เกิดข้อผิดพลาดในการบันทึกยอด' }) }
  }

  const handleStartTelegramProcess = () => {
    if (fullReportData.length === 0) return setTelegramModal({ isOpen: true, type: 'error', message: 'ไม่มีข้อมูลในวันที่เลือกครับ' })
  ...
  const handleConfirmAndSend = async (finalOrderItems: any[]) => {
    ...
    try {
      ...
      for (const item of finalOrderItems) {
        ...
        if (item.check_id) {
          await supabase.from('daily_stock_checks').update({ actual_order_qty: qty }).eq('id', item.check_id);
        }
      }
      
      fetchData(); 

      const dataToSend = fullReportData.map(originalItem => {
      ...
      if (result.success) {
        ...
        setTelegramModal({ isOpen: true, type: 'error', message: result.error || 'เกิดข้อผิดพลาด' })
      }
    } catch (error) {
      setTelegramModal({ isOpen: true, type: 'error', message: 'เชื่อมต่อ Telegram ไม่ได้' })
    } finally {
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto relative">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
        ...
        <div className="flex flex-col sm:flex-row gap-2.5 w-full sm:w-auto">
          <button onClick={handleStartTelegramProcess} disabled={isTelegramSubmitting} className="bg-[#0088cc] hover:bg-[#0077b5] text-white px-5 py-2.5 rounded-full text-sm font-bold shadow-md transition-all flex items-center justify-center gap-2 w-full sm:w-auto">
            <span>✈️</span> ส่งรายงานเข้า Telegram
          </button>
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 mb-8">
            <div className="bg-white rounded-2xl shadow-sm border border-[#fecaca] overflow-hidden flex flex-col h-[400px]">
              ...
              <div className="overflow-y-auto flex-1 custom-scrollbar"><table className="w-full text-center border-collapse"><thead className="sticky top-0 bg-white shadow-sm"><tr className="text-sm border-b border-gray-100"><th className="p-4 text-left font-bold text-gray-700">ชื่อสินค้ารายการ</th><th className="p-4 font-bold text-gray-700">เหลืออยู่</th><th className="p-4 font-bold text-[#e11d48] bg-red-50/50">ต้องสั่งเพิ่ม</th></tr></thead><tbody>{filteredOrderItems.length === 0 ? (<tr><td colSpan={3} className="p-12 text-gray-400">✅ ไม่มีสินค้าที่ต้องสั่งเพิ่มในหมวดนี้</td></tr>) : (filteredOrderItems.map((item, idx) => (
                <tr key={idx} className="border-b border-gray-50 hover:bg-red-50/30 group">
                  <td className="p-4 text-left"><div className="font-bold text-gray-800">{item.name}</div><div className="flex gap-1 mt-1"><span className="text-[10px] font-medium text-red-700 bg-red-100 px-2 py-0.5 rounded-md">🏷️ {item.category}</span></div></td>
                  ...
                  <td className="p-2 sm:p-4 bg-red-50/50 flex flex-col justify-center items-center h-full min-h-[90px]">
                    {editingOrderId === item.product_id ? (
                      <div className="flex flex-col items-center gap-1.5">
                        <input type="number" step="any" autoFocus className="w-14 sm:w-16 border-2 border-[#df2323] rounded-lg p-1 sm:p-1.5 text-center font-bold text-[#df2323] focus:outline-none shadow-sm text-sm sm:text-base" value={editOrderQty} onChange={(e) => setEditOrderQty(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSaveOrderQty(item.check_id, item.product_id)} />
                        <div className="flex gap-1">
                          <button onClick={() => handleSaveOrderQty(item.check_id, item.product_id)} className="bg-[#df2323] text-white text-[10px] px-2 py-1 rounded font-bold shadow-sm">บันทึก</button>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-[#a7f3d0] overflow-hidden flex flex-col h-[400px]"><div className="bg-[#059669] p-4 px-6 text-white font-bold flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2">📦 รายการของเข้า</div><div className="flex items-center gap-2"><button onClick={() => openFilterModal('incoming')} className="text-xs bg-[#047857] text-white border border-white/20 rounded-full py-1.5 px-4 focus:outline-none shadow-inner font-medium cursor-pointer hover:bg-[#065f46] transition-colors flex items-center gap-1.5"><span>{incomingFilter}</span><span className="text-[10px]">▼</span></button><div className="text-xs bg-[#065f46] px-3 py-1.5 rounded-full">{filteredIncomingItems.length} รายการ</div></div></div><div className="overflow-y-auto flex-1 custom-scrollbar"><table className="w-full text-center border-collapse"><thead className="sticky top-0 bg-white shadow-sm"><tr className="text-sm border-b border-gray-100"><th className="p-4 text-left font-bold text-gray-700">ชื่อสินค้ารายการ</th><th className="p-4 font-bold text-[#059669] bg-green-50/50">จำนวนที่เข้า</th></tr></thead><tbody>{filteredIncomingItems.length === 0 ? (<tr><td colSpan={2} className="p-12 text-gray-400">ยังไม่มีรายการรับของเข้าในหมวดนี้</td></tr>) : (filteredIncomingItems.map((item, idx) => (<tr key={idx} className="border-b border-gray-50 hover:bg-green-50/30"><td className="p-4 text-left"><div className="font-bold text-gray-800">{item.name}</div><div className="flex mt-1"><span className="text-[10px] font-medium text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-md">🏷️ {item.category}</span></div></td><td className="p-4 bg-green-50/50"><div className="font-bold text-[#059669] text-2xl">+{item.incoming}</div><div className="text-xs text-gray-500">{item.unit}</div></td></tr>)))}</tbody></table></div></div>
            <div className="bg-white rounded-2xl shadow-sm border border-[#bfdbfe] overflow-hidden flex flex-col h-[400px]"><div className="bg-[#2563eb] p-4 px-6 text-white font-bold flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2">📊 สต๊อกคงเหลือปัจจุบัน</div><div className="flex items-center gap-2"><button onClick={() => openFilterModal('balance')} className="text-xs bg-[#1d4ed8] text-white border border-white/20 rounded-full py-1.5 px-4 focus:outline-none shadow-inner font-medium cursor-pointer hover:bg-[#1e40af] transition-colors flex items-center gap-1.5"><span>{balanceFilter}</span><span className="text-[10px]">▼</span></button><div className="text-xs bg-[#1d4ed8] px-3 py-1.5 rounded-full">{filteredBalanceItems.length} รายการ</div></div></div><div className="overflow-y-auto flex-1 custom-scrollbar"><table className="w-full text-center border-collapse"><thead className="sticky top-0 bg-white shadow-sm"><tr className="text-sm border-b border-gray-100"><th className="p-4 text-left font-bold text-gray-700">ชื่อสินค้ารายการ</th><th className="p-4 font-bold text-[#2563eb] bg-blue-50/50">ยอดคงเหลือ</th></tr></thead><tbody>{filteredBalanceItems.length === 0 ? (<tr><td colSpan={2} className="p-12 text-gray-400">ยังไม่มีรายการสินค้าในหมวดนี้</td></tr>) : (filteredBalanceItems.map((item, idx) => (<tr key={idx} className="border-b border-gray-50 hover:bg-blue-50/30"><td className="p-4 text-left"><div className="font-bold text-gray-800">{item.name}</div><div className="flex mt-1"><span className="text-[10px] font-medium text-blue-700 bg-blue-100 px-2 py-0.5 rounded-md">🏷️ {item.category}</span></div></td><td className="p-4 bg-blue-50/50"><div className="font-bold text-[#2563eb] text-2xl">{item.balance}</div><div className="text-xs text-gray-500">{item.unit}</div></td></tr>)))}</tbody></table></div></div>
            <div className="bg-white rounded-2xl shadow-sm border border-[#fef08a] overflow-hidden flex flex-col h-[400px]"><div className="bg-[#eab308] p-4 px-6 text-white font-bold flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2">📉 ปริมาณที่ถูกใช้ไป</div><div className="flex items-center gap-2"><button onClick={() => openFilterModal('used')} className="text-xs bg-[#ca8a04] text-white border border-white/20 rounded-full py-1.5 px-4 focus:outline-none shadow-inner font-medium cursor-pointer hover:bg-[#a16207] transition-colors flex items-center gap-1.5"><span>{usedFilter}</span><span className="text-[10px]">▼</span></button><div className="text-xs bg-[#ca8a04] px-3 py-1.5 rounded-full">{filteredUsedItems.length} รายการ</div></div></div><div className="overflow-y-auto flex-1 custom-scrollbar"><table className="w-full text-center border-collapse"><thead className="sticky top-0 bg-white shadow-sm"><tr className="text-sm border-b border-gray-100"><th className="p-4 text-left font-bold text-gray-700">ชื่อสินค้ารายการ</th><th className="p-4 font-bold text-[#ca8a04] bg-yellow-50/50">ปริมาณที่ถูกใช้ไป</th></tr></thead><tbody>{filteredUsedItems.length === 0 ? (<tr><td colSpan={2} className="p-12 text-gray-400">ยังไม่มีประวัติการใช้วัตถุดิบในหมวดนี้</td></tr>) : (filteredUsedItems.map((item, idx) => (<tr key={idx} className="border-b border-gray-50 hover:bg-yellow-50/30"><td className="p-4 text-left"><div className="font-bold text-gray-800">{item.name}</div><div className="flex mt-1"><span className="text-[10px] font-medium text-yellow-700 bg-yellow-100 px-2 py-0.5 rounded-md">🏷️ {item.category}</span></div></td><td className="p-4 bg-yellow-50/50"><div className="font-bold text-[#ca8a04] text-2xl">-{item.used}</div><div className="text-xs text-gray-500">{item.unit}</div></td></tr>)))}</tbody></table></div></div>
          </div>
        </>
      )}

      {isFilterModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200"><div className="bg-gray-50 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col border border-gray-200"><div className={`${themeBg} p-4 px-6 flex justify-between items-center text-white`}><h2 className="text-lg font-bold flex items-center gap-2">🔍 {filterTitle}</h2><button onClick={() => setIsFilterModalOpen(false)} className="bg-white/20 hover:bg-white/30 rounded-full w-8 h-8 flex items-center justify-center transition-colors"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg></button></div><div className="p-6"><p className="text-sm font-bold text-gray-700 mb-4">เลือกหมวดหมู่ที่ต้องการดู:</p><div className="flex flex-wrap gap-2.5"><button onClick={() => handleSelectFilter('ทั้งหมด')} className={`px-4 py-2 border rounded-xl font-bold shadow-sm transition-all text-sm ${currentSelectedFilter === 'ทั้งหมด' ? 'border-transparent text-white' : 'border-gray-200 text-gray-700 hover:border-gray-300 hover:bg-gray-100 bg-white'}`} style={currentSelectedFilter === 'ทั้งหมด' ? { backgroundColor: themeColor } : {}}>รวมทั้งหมด</button>{currentModalCategories.map(cat => (<button key={cat} onClick={() => handleSelectFilter(cat)} className={`px-4 py-2 border rounded-xl font-bold shadow-sm transition-all text-sm ${currentSelectedFilter === cat ? 'border-transparent text-white' : 'border-gray-200 text-gray-700 hover:border-gray-300 hover:bg-gray-100 bg-white'}`} style={currentSelectedFilter === cat ? { backgroundColor: themeColor } : {}}>{cat}</button>))}</div></div></div></div>
      )}
      ...
      <OrderPreviewModal isOpen={isPreviewOpen} onClose={() => setIsPreviewOpen(false)} onConfirm={handleConfirmAndSend} items={fullReportData} isLoading={isTelegramSubmitting} />
